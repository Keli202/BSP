...
# ----------- 1. 设置路径 -----------
TRAIN_DIR = 'Datasets/processed_features/features/train'
TEST_DIR = 'Datasets/processed_features/features/test'
SAVE_DIR = 'feature_analysis_output'
...
def load_features_labels(feature_dir):
    ...
    for file in sorted(os.listdir(feature_dir)):
        ...
        feats, labels = data['features'], data['labels']  # feats: (T, 9, 768)
        if feats.ndim == 3:
            feats = feats[:, -1, :]  # 只取第11层CLS特征 (T, 768)
        elif feats.ndim == 2:
            pass  # 已经是 (T, 768)，直接用
        else:
...
print(f"\n[Test自分类] KNN accuracy: {acc_self:.4f}")
...
with open(os.path.join(SAVE_DIR, "knn_self_classification_report.txt"), "w") as f:
    f.write("[Test自分类] KNN self-classification accuracy: %.4f\n" % acc_self)
    f.write(report_self)
    f.write("\nConfusion Matrix:\n")
    f.write(str(cm_self))

# ----------- 3. KNN分开训练（Train集fit，Test集predict） -----------
