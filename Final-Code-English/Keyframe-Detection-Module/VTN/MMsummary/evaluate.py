...
# -*- coding: utf-8 -*-
"""
MMsummary — Evaluation
----------------------
Evaluates a trained VideoTransformer on **basic (no-highlight)** features.
Reads NPZs from:
  $DATA_ROOT/processed_features/features/{SPLIT}/*.npz
and writes metrics/plots per case to:
  ./eval_output/{SPLIT}/

Override with env vars:
  PROJECT_ROOT, DATA_ROOT, DATA_DIR, MODEL_PATH
"""
...
DEFAULT_DATA_DIR = DATA_ROOT / "processed_features" / "features" / SPLIT
...
def compute_cosine_similarity(features, true_labels, pred_labels):
    """
    Compute bi-directional mean of maxima between predicted and true keyframe features.
    Returns (mean_similarity, full_matrix) or (None, None) if no keyframes.
    """
...
for file in sorted(os.listdir(DATA_DIR)):
    ...
    # save per-frame probabilities (requested by supervisor)
    df = pd.DataFrame({
    ...
    # similarity heatmap (if both sides have keyframes)
...
# -------- Global metrics over all frames --------
...
# -------- Mean cosine similarity across cases with keyframes --------
if all_mean_sim:
