...
# -*- coding: utf-8 -*-
"""
Quick sanity checks on FPN features.
Reads:
  data/processed_features/features/{train,test}/*.npz
Outputs to: feature_analysis_output/
If features are (T, 9, 768), we take the last layer (block 11) for plots/KNN.
"""
...
def load_features_labels(feature_dir):
    ...
    for file in sorted(os.listdir(feature_dir)):
        ...
        if feats.ndim == 3:
            feats = feats[:, -1, :]  # take block 11: (T, 768)
        elif feats.ndim == 2:
            pass
        else:
